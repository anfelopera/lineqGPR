...
k1gaussian <- function(x1, x2, par, d = 1) {
  if (!is.matrix(x1) || ncol(x1) != d)
    x1 <- matrix(x1, ncol = 1)
  if (!is.matrix(x2) || ncol(x2) != d)
    x2 <- matrix(x2, ncol = 1)
  sigma2 <- par[1]
  theta <- par[2]
  ...
  attr(kern, "derivative") <- list(x1 = d1x1, x2 = -d1x1, x1x1 = d2x1, x1x2 = -d2x1, x2x2 = d2x1, x2x1 = -d2x1, x1x1x1 = d3x1,
                                   x1x1x2 = -d3x1, x2x2x1 = d3x1, x1x1x1x1 = d4x1, x1x1x2x2 = d4x1)
  return(kern)
}
...
k2gaussian <- function(x1, x2, par, d = 2) {
  ...
  # gradients
  attr(kern, "gradient") <- list(sigma2 = kern/sigma2, theta_x1 = 2 * kern * dist2_x1/theta_x1, theta_x2 = 2 * kern *
                                   dist2_x2/theta_x2)
  return(kern)
}
...
k1matern32 <- function(x1, x2, par, d = 1) {
  ...
  attr(kern, "derivative") <- list(x1 = d1x1, x2 = -d1x1, x1x1 = d2x1, x1x2 = -d2x1, x2x2 = d2x1, x2x1 = -d2x1)
  return(kern)
}
...
k1matern52 <- function(x1, x2, par, d = 1) {
  ...
  # gradients
  dsigma2 <- kern/sigma2
  dtheta <- -(sigma2/theta) * (sqrt5distSgn + (10/3) * dist2) * expDistSgn + (sqrt5distSgn/theta) * kern
  attr(kern, "gradient") <- list(sigma2 = dsigma2, theta = dtheta)
